...
Original file is located at
    https://colab.research.google.com/drive/1qmp_bmWnrzhaZcffSPUZ2NUN8ACJGrDa
"""
...
#topology = model.get_weights()
#WeightsFlattened=RecFlatten(topology)

#initialize population and find its fitness
for i in range(S):
    ks.clear_session()
...
print(NNs_Population[0])
print("Secondo:")
print(NNs_Population[1])

for i in range(0,N):#Termination condition
    for j in range(0, int(C*S)):
        ...
        Child1 = DeFlattenWTopology(model,ChildrenWeights[0])
        Child2 = DeFlattenWTopology(model,ChildrenWeights[1])
        NNs_Population.append((Child1,-1))
        NNs_Population.append((Child2,-1))

    NNs_lenght = len(NNs_Population)
    for i in range(NNs_lenght):
        new_tuple = (NNs_Population[i][0],np.random.uniform())
